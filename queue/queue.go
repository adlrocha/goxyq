...
func runJob(pool *redis.Pool, name string, handler func([]byte) ([]byte, error)) (bool, error) {
	// Create and close connection when finished
	c := pool.Get()
	defer c.Close()
...
func sampleHandler(job []byte) ([]byte, error) {
	fmt.Println("Sample handler function for job with content:", string(job))
	return job, nil
}
...
func main() {
	...
	fmt.Println(createJob(pool, "testQueue", []byte("value")))
	fmt.Println(getQueue(pool, "testQueue"))
	fmt.Println(runJob(pool, "testQueue", sampleHandler))
	fmt.Println(getQueue(pool, "testQueue"))
